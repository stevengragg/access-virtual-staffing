        ...
        <Card className="w-full border border-gray-200">
            ...
            <div className="flex gap-1 mt-2 px-2">
                {Array.from({ length: maxValue }, (_, index) => (
                    <div
                        key={index}
                        className={`h-2 flex-1 rounded ${index < currentValue ? "bg-primaryBrightAqua" : "bg-gray-300"}`}
                    />
                ))}
            </div>
            <div className="flex p-4 items-center justify-between gap-2">
                ...
                <Button variant="ghostBlue" onClick={()=> router.push('/app/profile')} size="sm" className="text-primaryBrightAqua">Complete your profile</Button>
            </div>
        </Card>
    );
};
